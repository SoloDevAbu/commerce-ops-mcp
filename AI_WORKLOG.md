...
I cross-checked this with a second model and got the same conclusion,
which confirmed it wasn't a case of me disagreeing with a genuinely
sound recommendation — the monorepo suggestion was solving a problem this
...
for local development, but reconsidered it for the *deployed* target:
...
- **Unit/integration tests (Vitest)** against the service layer directly,
  ...
  stuck-in-processing timeout, and the clean happy path) plus the
  idempotency guard on the retry operation and input validation on the
  status-update operation. This verifies business logic correctness
